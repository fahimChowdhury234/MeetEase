        ...
        <div class="w-3/4 mx-auto" id="box">
            <div class="flex flex-col w-full justify-between lg:flex-row">
                <div class="w-full lg:w-[65%] relative">
                    <div id="userVideo" class="video border-2 border-prim-dark rounded-lg h-[60vh] w-full relative "
                        v-show="!video.mute">
                    ...
                    <div v-show="video.mute"
                        class=" border-2 border-prim-dark bg-gray flex justify-center items-center flex-col gap-5 text-white text-base rounded-lg h-[60vh] w-full">
                    ...
                    <div
                        class="controler flex items-center gap-5 justify-center mt-5 absolute right-1/2 lg:right-0 bottom-0 transform translate-x-1/2 -translate-y-1/2 lg:relative lg:transform-none">
                ...
                <!-- <div v-for="(part, index) in participants" :key="'media' + index" class="Camera">
                    <MeetingMediaBox :part="part" :ref="'meetingBox' + index" :index="index" />
                </div> -->
                <div v-if="participants.length > 0" class="w-full lg:w-[25%] h-[90vh] mt-4 overflow-y-auto lg:mt-0">
                    <div class="border-2 border-prim-dark rounded-lg h-52 w-full mb-4  relative" id="remot"
                        v-show="!remoteUserVideo">
                        <span class="absolute top-0 left-0 bg-prim text-white text-sm capitalize px-2 py-1 rounded-md z-10">
                            {{ remoteUserName }}
                        </span>
                    </div>
                    <div v-show="remoteUserVideo"
                        class="border-2 border-prim-dark bg-gray flex justify-center items-center flex-col gap-5 text-white text-base rounded-lg  h-52 w-full mb-4 ">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-16 ">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                ...
                <div v-else class="w-full lg:w-[25%] mt-4 flex items-center justify-center lg:mt-0">
                    <p> Hello <strong class="capitalize text-prim">{{ uName }}</strong> ,Only you are in the room</p>
                </div>
            </div>
    ...
    methods: {
        ...
        async connectToRoom(eventsCallback) {
            ...
            await agoraEngine.join(
                this.appId, this.channel, this.agora_token, this.uid
            );
