      ...
      <div class="container px-5 flex items-center py-4 mx-auto justify-between relative lg:w-[1350px]">
        ...
        <div class="absolute top-16 right-6 bg-white border border-gray border-opacity-50 px-5 py-5 rounded-lg" v-if="isOpen">
          <ul class="flex items-start flex-col gap-6 pl-4">
            <router-link to="/" class="text-sm rounded-md font-medium text-gray cursor-pointer transition-all hover:text-prim" :class="currentUser ? 'hidden' : ''" href="#">Home</router-link>
            <router-link to="/login" class="text-sm rounded-md font-medium text-gray cursor-pointer transition-all hover:text-prim" :class="currentUser ? 'hidden' : ''" href="#">Log in</router-link>
          </ul>
          <div class="mt-4" v-if="!currentUser">
            <router-link to="/signup">
              <button class="signup flex items-center gap-1 justify-center w-32 text-sm py-2 rounded-full font-medium border bg-prim text-white transition-all hover:bg-prim-dark active:bg-prim">
                <span>Sign up </span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                  <path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd" />
                </svg>
          ...
          <div class="mt-4" v-else>
            <button class="signup flex items-center gap-1 justify-center w-32 text-sm py-2 rounded-full font-medium border bg-prim text-white transition-all hover:bg-prim-dark active:bg-prim" @click="logOut">
              <span>Log Out </span>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                <path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd" />
              </svg>
